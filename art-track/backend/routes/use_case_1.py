...
def use_case_1():
    ...
    if selected_museums:
        museums_filter = " || ".join([f'?musuem = "{museum}"' for museum in selected_museums])
        sparql_filters.append(f"FILTER({museums_filter})")

    # Handle mandatory Medium filter
    if selected_medium:
        sparql_filters.append(f'FILTER regex(?medium, "{selected_medium}", "i")')

    # Handle optional Date Created filter
    if start_year and end_year:
        sparql_filters.append(f'FILTER(xsd:integer(?dateCreated) >= {start_year} && xsd:integer(?dateCreated) <= {end_year})')
    ...
    try:
        ...
        if not raw_results.get("results") or not raw_results["results"].get("bindings"):
            ...
            return jsonify({"error": "No results match your query. Please adjust filters."}), 404
        ...
        for result in raw_results["results"]["bindings"]:
          ...
          if "datatype" in title_data and title_data["datatype"] == "http://www.semanticweb.org/rachi/ontologies/2024/10/artTrack#string":
          ...
          if dimensions != "N/A":
              dimensions = dimensions.replace("×", "*").replace("\xa0", "").strip()
